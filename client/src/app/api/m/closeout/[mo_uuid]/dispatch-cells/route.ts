...
export async function GET(
  ...
  try {
    const data = await api(
      `/api/m/closeout/${encodeURIComponent(mo_uuid)}/dispatch-cells`,
      { token },
    );
    return NextResponse.json(data);
  } catch (err) {
    const { payload, status } = toJsonError(err, {
      source: "proxy:/api/m/closeout/[mo_uuid]/dispatch-cells",
      fallbackDetail: "Couldn't load the dispatch cells.",
    });
