...
/**
 * GET /api/three-pl/returns?q=&cursor=&limit=
 *
 * Client-side proxy for the mobile 3PL hub's Return tab.
 */
export async function GET(req: Request) {
  ...
  try {
    ...
    const url = "/api/three-pl/returns" + (qs ? `?${qs}` : "");
    const data = await api(url, { token: t });
    return NextResponse.json(data);
  } catch (err) {
    const { payload, status } = toJsonError(err, {
      source: "proxy:/api/three-pl/returns GET",
      fallbackDetail: "Couldn't load the return queue.",
    });
