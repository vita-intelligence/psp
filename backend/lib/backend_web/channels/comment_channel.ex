defmodule BackendWeb.CommentChannel do
  ...
  defp check_view_perm(user, "pricelist"),
    do: gate(user, "pricelists.view")

  defp check_view_perm(user, "customer_order"),
    do: gate(user, "customer_orders.view")

  defp check_view_perm(user, "customer_invoice"),
    do: gate(user, "customer_invoices.view")
  ...
  defp check_view_perm(user, "equipment"),
    do: gate(user, "equipment.view")

  defp check_view_perm(user, "hr_employee"),
    do: gate(user, "hr.view")
  ...
  defp resolve_entity_id(user, "pricelist", uuid) do
    case Backend.Pricelists.get_for_company(user.company_id, uuid) do
      %{id: id} -> {:ok, id}
      _ -> {:error, :not_found}
    end
  end

  defp resolve_entity_id(user, "customer_order", uuid) do
    case Backend.CustomerOrders.get_for_company(user.company_id, uuid) do
      %{id: id} -> {:ok, id}
      _ -> {:error, :not_found}
    end
  end

  defp resolve_entity_id(user, "customer_invoice", uuid) do
    case Backend.CustomerInvoices.get_for_company(user.company_id, uuid) do
      %{id: id} -> {:ok, id}
      _ -> {:error, :not_found}
    end
  end
  ...
  defp resolve_entity_id(user, "equipment", uuid) do
    case Backend.Equipment.get_for_company(user.company_id, uuid) do
      %{id: id} -> {:ok, id}
      _ -> {:error, :not_found}
    end
  end

  defp resolve_entity_id(user, "hr_employee", uuid) do
    case Backend.HR.get_employee(user.company_id, uuid) do
      %{id: id} -> {:ok, id}
      _ -> {:error, :not_found}
    end
  end
