defmodule Backend.Warehouses.StorageTags do
  ...
  def list_page(company_id, opts \\ []) do
    ...
    {code_id, column_filter} =
      ListQueries.pop_code_column_filter(opts[:column_filter], company_id, "storage_tag")

    base =
      StorageTag
      |> where([t], t.company_id == ^company_id)
      |> ListQueries.apply_search(opts[:search], @search_fields, {company_id, "storage_tag"})
  ...
  defp maybe_code_id_filter(query, :no_match), do: where(query, [t], false)
  defp maybe_code_id_filter(query, id) when is_integer(id),
    do: where(query, [t], t.id == ^id)
