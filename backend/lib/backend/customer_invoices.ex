defmodule Backend.CustomerInvoices do
  ...
  def list_page(company_id, opts \\ []) when is_integer(company_id) do
    ...
    {code_id, column_filter} =
      ListQueries.pop_code_column_filter(column_filter, company_id, "customer_invoice")

    base =
      CustomerInvoice
      |> where([i], i.company_id == ^company_id)
      |> ListQueries.apply_search(opts[:search], @invoice_search, {company_id, "customer_invoice"})
